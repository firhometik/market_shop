...
$.fn.confirm = function(option){ 
				...
				if(e.status === -1)
				{
					swal(
						...
						{   
							if(isConfirm)
							{ 				  
								$.getJSON(e.url,function(re){msg(re,_this)})
							}else
							{
								if(option.cancel) option.cancel(_this);
							}
						}
					);
				}else
	...
	function msg(re,_this)
	{
		if(re.status)
		{
			if(option.success)
			{
				option.success(re,_this)
			}else
			{
				swal({title:"Good job!",text: re.info,type: "success",timer: 2000},function(isConfirm){
