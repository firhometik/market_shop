...
class User extends Controller
{
	...
	{ 
		...
		//判断帐号
		$user = UserModel::getUserinfo('o2o_bis','',$where);
		...
		cookie('bis_key',$key,array('expire'=>3600*24*7,"path"=>"/","domain"=>get_domain()));
        ...
        if($re == "ok")
        {
        	session("bis_user",array("id" => $user["id"],"key" => $key));
	        session("bis_sessid",$sessionid);
	...
	{ 
		...
		if(session("bis_user")) //session验证登陆
		{
		...
		}else//cookie验证登陆
		{
	    	...
	    	if($user)
	    	{
	    		session("bis_user",$session = array("id" => $user['id'],"key" => $code));
	    	}
		}
		if(!$user)
        {
            session("bis_user",null);
            session("errya_sessid",null);
            cookie("bis_user",NULL,array('expire'=>3600*24*7,"path"=>"/","domain"=>get_domain()));
	...
	*/
	public static function getLoginData($type="check_pass",$user)
	{
		...
		{ 
			...
			$memberid = session("bis_user.memberid");
			if($memberid)
			{
			...
			if($num % $id == $mod)
			{
				$userTemp = UserModel::getUserinfo('o2o_bis','',$where);
